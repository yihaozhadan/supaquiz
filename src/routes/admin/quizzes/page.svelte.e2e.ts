...
test.describe('Quiz CRUD Operations', () => {
	...
	test('should create a new quiz', async ({ page }) => {
		...
		await page.waitForURL(/\/admin\/quizzes\/.*\/edit/);
	...
	test('should edit quiz settings', async ({ page }) => {
		...
		await expect(page.locator('.bg-green-50')).toContainText('Quiz updated successfully');
	});
	...
	test('should duplicate a quiz', async ({ page }) => {
		...
		await expect(page.locator('.bg-green-50')).toContainText('Quiz duplicated successfully');
	});
	...
	test('should delete a quiz', async ({ page }) => {
		...
		await expect(page.locator('.bg-green-50')).toContainText('Quiz deleted successfully');
	});

	test('should enforce max 5 active quizzes constraint', async ({ page }) => {
		// Try to activate more than 5 quizzes
		await page.goto('/admin/quizzes');
		...
		// This test would need setup with existing quizzes to fully test the constraint
...
test.describe('Question Editor', () => {
	...
	test('should add MCQ single choice question', async ({ page }) => {
		...
		await page.waitForURL(/\/admin\/quizzes\/.*\/questions/);
		...
		await expect(page.locator('.bg-green-50')).toContainText('Question created successfully');
	});
